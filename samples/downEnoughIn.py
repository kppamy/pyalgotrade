...
class DownEnoughIn(strategy.BacktestingStrategy):
    def __init__(self, feed, instrument, window=3):
        ...
        self.__pricePCT = roc.RateOfChange(feed[instrument].getPriceDataSeries(), 1).getValues()
        self.__longPos = None
...
feed = yahoofeed.Feed()
instrument = "lpyl"
feed.addBarsFromCSV(instrument, "lpyl.csv")
...
if plot:
    ...
    # plt.getInstrumentSubplot(instrument).addDataSeries("pri", strat.getEntrySMA())
...
if plot:
    plt.plot(fromDateTime=datetime.strptime('2017-07-01', '%Y-%m-%d'),
             toDateTime=datetime.strptime('2018-03-01', '%Y-%m-%d'))
